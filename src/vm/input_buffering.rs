...
extern "C" fn handle_interrupt(_sig: c_int) {
 ...
 pub fn setup() {
    ...
    // Disable canonical mode and echo (input buffering)
    disable_input_buffering(); // make sure this is implemented using termios
}
...
pub fn check_key() -> bool {
    unsafe {
        ...
        select(fd + 1, &mut readfds, std::ptr::null_mut(), std::ptr::null_mut(), &mut timeout) > 0
    }
}
